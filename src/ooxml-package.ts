...
export class OOXMLPackage {
   ...
   */
  constructor(private file: Uri, private treeDataProvider: OOXMLTreeDataProvider, context: ExtensionContext) {
    this.zip = new JSZip();
    this.rootFileNode = FileNode.create(basename(this.ooxmlFilePath), this.treeDataProvider.rootFileNode, this);
    this.rootFileNode.isOoxmlPackage = true;
    this.treeDataProvider.refresh();
    this.cache = new OOXMLFileCache(this.ooxmlFilePath, context);
    this.openOoxmlPackage(file);
  }
    ...
    try {
      await window.withProgress(
        ...
        async progress => {
          ...
          await commands.executeCommand('vscode.open', Uri.file(fileCachePath));
        },
      );
    } catch (err) {
   ...
   * @method getDiff
   * @async
   * @param  {string} filePath the path of FileNode to be diffed
   * @returns {Promise<void>}
   */
   ...
   * @returns Promise{void}
   */
  private async tryFormatDocument(filePath: string): Promise<void> {
   ...
   * @returns {Promise<void>}
   */
  private async openOoxmlPackage(file: Uri): Promise<void> {
    try {
      await window.withProgress(
        {
          location: ProgressLocation.Notification,
  ...
  private async populateOOXMLViewer(files: { [key: string]: JSZip.JSZipObject }, isFirstOpen: boolean): Promise<void> {
    ...
    for (const fileWithPath of fileKeys) {
      ...
      let currentFileNode = this.rootFileNode;
      ...
      for (const fileOrFolderName of names) {
        ...
        if (existingFileNode) {
          ...
          const newFileNode = FileNode.create(fileOrFolderName, currentFileNode, this);
  ...
  private async handleDeletedParts(): Promise<void> {
    try {
      ...
      const fileNodeQueue = [this.rootFileNode];
